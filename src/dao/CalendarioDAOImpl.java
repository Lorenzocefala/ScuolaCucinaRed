...
public class CalendarioDAOImpl implements CalendarioDAO {
	...
	public void insert(Edizione ed) throws SQLException {
		...
		PreparedStatement ps = conn
				.prepareStatement("insert into calendario(id_corso,dataInizio,durata,aula,docente) values (?,?,?,?,?)");
	...
	public void update(Edizione ed) throws SQLException {
		PreparedStatement ps = conn.prepareStatement(
				"update calendario set dataInizio=?, durata=?, aula=?, docente=? where id_edizione= ?");
		...
		int n = ps.executeUpdate();
		if (n == 0)
			throw new SQLException("edizione " + ed.getCodice() + " non presente");
	...
	/*
	 * lettura di tutte le edizioni presenti nel calendario dei corsi se non vi sono
	 * edizioni registrate viene ritornata una lista vuota
	 */
	@Override
	...
	/*
	 * lettura di tutte le edizioni a cui un certo utente � iscritto o � stato
	 * iscritto in passato (vale a dire tutte), presenti nel calendario dei corsi le
	 * edizioni vengono individuate in base al idUtente dell'utente se non vi sono
	...
	public ArrayList<Edizione> select(String idUtente) throws SQLException {
		...
		ArrayList<Edizione> edizioni = new ArrayList<Edizione>();
		PreparedStatement ps = conn.prepareStatement(
				"select calendario.id_edizione,id_corso,id_utente,dataInizio,durata,aula,docente from calendario,iscritti where calendario.id_edizione=iscritti.id_edizione and iscritti.id_utente=?");
	...
	/*
	 * leggere tutte le edizioni presenti nel calendario nel range delle date da, a
	 * (inclusi) se non vi sono edizioni in quel range di date viene ritornata una
	 * lista vuota
	 */
	@Override
	public ArrayList<Edizione> select(java.util.Date da, java.util.Date a) throws SQLException {
		ArrayList<Edizione> edizioni = new ArrayList<Edizione>();
		PreparedStatement ps = conn
				.prepareStatement("select * from calendario where dataInizio beetween dataInizio=? and dataFine=?");
		ResultSet rs = ps.executeQuery();
	 ...
	 * partire dalla data in odierna e dell'anno corrente se future = false devono
	 * essere lette tutte le edizioni le edizioni vengono individuate in base al
	 * idCategoria se non vi sono edizioni per quella categoria o la categoria non
	 ...
	 * true, le edizioni lette devono essere solo quelle a partire dalla data in
